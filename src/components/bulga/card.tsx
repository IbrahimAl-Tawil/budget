// Bulga surface card — the calm, flat replacement for the old GlassCard.
...
export function Card({ className, hover = false, children, ...props }: CardProps) {
  return (
    <div
      className={cn(
        "rounded-[20px] border border-[var(--color-bk-line)] bg-[var(--color-bk-surface)]",
        hover &&
...
export function CardLabel({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "text-[12px] font-semibold tracking-[0.07em] uppercase text-[var(--color-bk-faint)]",
        className
      )}
      {...props}
    >
