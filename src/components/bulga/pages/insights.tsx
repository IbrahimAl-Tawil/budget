...
export function BulgaInsights({ insights: initial, accent, theme, currency }: BulgaInsightsProps) {
  ...
  // Phones document-scroll the shell, so bk-fullbleed can't resolve a height
  ...
  useEffect(() => {
    ...
    const bar = document.querySelector(".bk-topbar");
  ...
  return (
    <div
      ref={pageRef}
      className="bk-enter bk-fullbleed"
      style={{
        display: "flex",
          ...
          <>
            ...
            <div style={{ width: 40, flexShrink: 0, display: "flex", justifyContent: "flex-end" }}>
              {view === "chat" ? (
                ...
                >
                  {generating ? <Loader2 size={16} className="bk-spin" /> : <Sparkles size={16} />}
                </Button>
              )}
            </div>
          </>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: "hidden" }}>
        ...
        >
          ...
          <div style={{ flex: "0 0 50%", height: "100%", pointerEvents: view === "insights" ? "auto" : "none" }} aria-hidden={view !== "insights"}>
            ...
            <div className="bk-scroll" style={{ height: "100%", overflowY: "auto", padding: mobile ? "20px 16px 24px" : "56px 24px 28px", background: "var(--color-bk-surface)" }}>
...
// button dismiss it; motion collapses under prefers-reduced-motion via bk-enter.
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", justifyContent: "flex-end" }}>
      ...
      <div
        ...
        className="bk-scroll bk-drawer-in"
        style={{
          position: "relative",
          ...
          overflowY: "auto",
          background: "var(--color-bk-surface)",
          borderLeft: "1px solid var(--color-bk-line)",
      ...
      >
        ...
        >
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ margin: "10px 0 0", fontSize: 13.5, lineHeight: 1.55, color: "var(--color-bk-muted)" }}>
              {insight.body}
            </p>
          </div>
        ...
        <div style={{ padding: "18px 22px 28px", flex: 1 }}>
          {loading ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 0", color: "var(--color-bk-muted)" }}>
              <Loader2 size={18} className="bk-spin" />
            </div>
          ) : !detail ? (
            <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: "var(--color-bk-muted)" }}>
...
function DetailBody({ detail, theme, currency }: { detail: InsightDetail; theme: BulgaTheme; currency: string }) {
  if (detail.kind === "category") {
    return (
      <>
        ...
        {detail.count === 0 ? (
          <p style={{ margin: "18px 0 0", fontSize: 13.5, color: "var(--color-bk-muted)" }}>
              ...
              <Section title="Where it comes from">
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {detail.byAccount.map((a) => (
                    ...
                    >
                      {a.account}
                      <span className="bk-num" style={{ fontWeight: 600, color: theme.accentDeep }}>{fmt(a.total, currency)}</span>
                    </span>
                  ))}
                </div>
              </Section>
            ...
            <Section title={`Largest transactions${(detail.count ?? 0) > 15 ? ` · top 15 of ${detail.count}` : ""}`}>
              <div style={{ display: "flex", flexDirection: "column" }}>
                {detail.transactions?.map((t, i) => (
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: 13.5, color: "var(--color-bk-ink)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t.name}</div>
                      <div style={{ fontSize: 11.5, color: "var(--color-bk-faint)", marginTop: 2 }}>
                        {t.date}{t.account ? ` · ${t.account}` : ""}
                      </div>
                    </div>
                    <span className="bk-num" style={{ fontSize: 13.5, fontWeight: 600, color: "var(--color-bk-ink)", flexShrink: 0 }}>{fmt(t.amount, currency)}</span>
                  </div>
                ))}
              </div>
            </Section>
      ...
      <Section title="Income vs. spending by month" first>
        <div style={{ display: "flex", flexDirection: "column" }}>
          {detail.months.map((m, i) => (
            ...
            >
              <span style={{ fontSize: 13, color: "var(--color-bk-ink)", fontWeight: 600 }}>{m.label}</span>
              <span className="bk-num" style={{ fontSize: 12.5, color: theme.accentDeep, textAlign: "right" }}>+{fmt(m.income, currency)}</span>
              <span className="bk-num" style={{ fontSize: 12.5, color: "var(--color-bk-clay)", textAlign: "right" }}>−{fmt(m.expenses, currency)}</span>
            </div>
          ))}
        </div>
      </Section>
...
function StatRow({ children, theme }: { children: ReactNode; theme: BulgaTheme }) {
  return (
    <div
      style={{
        display: "flex",
...
function Stat({ label, value, big }: { label: string; value: string; big?: boolean }) {
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--color-bk-faint)" }}>{label}</div>
      <div className="bk-num" style={{ marginTop: 4, fontSize: big ? 24 : 16, fontWeight: 600, color: "var(--color-bk-ink)", lineHeight: 1.1 }}>{value}</div>
    </div>
  );
}
...
function Section({ title, children, first }: { title: string; children: ReactNode; first?: boolean }) {
  return (
    <div style={{ marginTop: first ? 0 : 22 }}>
      <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--color-bk-muted)", marginBottom: 10 }}>{title}</div>
      {children}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      {rows.map(([k, v], i) => (
        ...
        >
          <span style={{ fontSize: 13, color: "var(--color-bk-muted)" }}>{k}</span>
          <span className="bk-num" style={{ fontSize: 13, fontWeight: 600, color: "var(--color-bk-ink)" }}>{v}</span>
        </div>
      ))}
    </div>
  );
...
function ViewToggle({ view, setView, theme, segW }: { view: View; setView: (v: View) => void; theme: BulgaTheme; segW: number }) {
  return (
    <div
      role="tablist"
