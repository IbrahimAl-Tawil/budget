...
// Bulga — a small, safe Markdown renderer for advisor answers.
//
...
function renderInline(text: string, keyPrefix: string): React.ReactNode[] {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    if (tok.startsWith("**")) {
      nodes.push(
        <strong key={`${keyPrefix}-b${k++}`} style={{ fontWeight: 600, color: "var(--color-bk-ink)" }}>
          {tok.slice(2, -2)}
        </strong>,
      );
    } else {
...
function renderBlockNode(b: Block, i: number, theme: BulgaTheme, animate: boolean): React.ReactNode {
  const cls = animate ? "bk-enter" : undefined;
  switch (b.type) {
    case "heading": {
      const size = b.level <= 2 ? 15.5 : b.level === 3 ? 14 : 13.5;
      return (
        <div key={i} className={cls} style={{ fontSize: size, fontWeight: 600, color: "var(--color-bk-ink)", marginTop: i === 0 ? 0 : 4 }}>
    ...
    case "hr":
      return <hr key={i} className={cls} style={{ border: 0, borderTop: "1px solid var(--color-bk-line-soft)", margin: "2px 0" }} />;
    case "ul":
      return (
...
export function AdvisorMarkdown({ content, theme }: { content: string; theme: BulgaTheme }) {
...
function Caret() {
  return <span className="bk-caret" aria-hidden />;
}
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (b.type === "paragraph") {
      ...
      if (shown < words.length) {
        ...
        nodes.push(
          <p key={i} className="bk-enter" style={{ margin: 0 }}>
...
function MarkdownTable({
  ...
  return (
    <div
      className={`bk-scroll${animate ? " bk-enter" : ""}`}
      style={{
        overflowX: "auto",
            ...
            <tr key={ri}>
              {headers.map((_, c) => {
                ...
                return (
                  <td
                    key={c}
                    className={numeric ? "bk-num" : undefined}
                    style={{
                      textAlign: aligns[c],
                      padding: "8px 12px",
                      color: "var(--color-bk-ink)",
                      borderBottom: ri === rows.length - 1 ? "none" : "1px solid var(--color-bk-line-soft)",
                      whiteSpace: "nowrap",
                    }}
                  >
