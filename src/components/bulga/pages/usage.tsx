// Bulga — internal AI USAGE page (/dev/usage). A server component (reads the DB
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      ...
      <div className="bk-num" style={{ fontSize: 24, letterSpacing: "-0.01em", color: "var(--color-bk-ink)" }}>
        {value}
      </div>
    </div>
  );
...
export function UsageView({ summary }: { summary: AiUsageSummary }) {
  ...
  return (
    <div className="bk-enter bk-page">
      ...
      >
        {users.length === 0 ? (
          <p style={{ margin: 0, padding: "36px 24px", textAlign: "center", fontSize: 14, color: "var(--color-bk-muted)" }}>
          ...
          <div className="bk-scroll" style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", minWidth: 720, borderCollapse: "collapse", fontSize: 13.5 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--color-bk-line)" }}>
                  <th style={TH}>User</th>
              ...
              <tbody>
                {users.map((u) => (
                  <tr key={u.userId} style={{ borderBottom: "1px solid var(--color-bk-line-soft)" }}>
                    <td style={{ padding: "12px 14px", minWidth: 0 }}>
                      <div style={{ fontWeight: 600, color: "var(--color-bk-ink)" }}>{u.name || "—"}</div>
                      <div style={{ fontSize: 12, color: "var(--color-bk-faint)", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {u.email || u.userId}
                      </div>
                    </td>
                    <td className="bk-num" style={{ ...TDNUM, color: "var(--color-bk-ink)" }}>{num(u.chats)}</td>
                    <td className="bk-num" style={{ ...TDNUM, color: "var(--color-bk-muted)" }}>{num(u.conversations)}</td>
                    <td className="bk-num" style={{ ...TDNUM, color: "var(--color-bk-muted)" }}>{num(u.insights)}</td>
                    <td className="bk-num" style={{ ...TDNUM, color: "var(--color-bk-muted)" }}>{num(u.inputTokens + u.outputTokens)}</td>
                    <td className="bk-num" style={{ ...TDNUM, fontWeight: 600, color: "var(--bk-accent, var(--color-primary))" }}>
                      {money(u.costUsd)}
                    </td>
                    <td style={{ ...TDNUM, color: "var(--color-bk-faint)", fontSize: 12.5 }}>{shortDate(u.lastUsedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <p style={{ margin: "14px 2px 0", fontSize: 11.5, color: "var(--color-bk-faint)" }}>
