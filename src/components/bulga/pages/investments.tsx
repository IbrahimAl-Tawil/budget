...
// Bulga — INVESTMENTS page (Blossom-inspired, our twist).
//
...
import { type BulgaTheme, accentFamilyTint, accentRamp } from "@/components/bulga/theme";
...
interface BulgaInvestmentsProps {
...
export function BulgaInvestments({
  accounts,
  holdings,
  accent,
  theme,
  ...
  const slideClass = navCount === 0 ? undefined : dir === 1 ? "bk-slide-in-right" : "bk-slide-in-left";
    ...
    <div style={CARD}>
      ...
      {hasHoldings ? (
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: "var(--color-bk-faint)", marginTop: 2 }}>{sub}</div>
                </div>
                <div style={{ textAlign: "right", flexShrink: 0 }}>
                  <div className="bk-num" style={{ fontSize: 15, fontWeight: 500, whiteSpace: "nowrap" }}>
                    {money(h.value)}
                  </div>
                  {h.live && h.dayChange != null ? (
                    <div className="bk-num" style={{ fontSize: 11, marginTop: 2, color: (h.dayChange ?? 0) >= 0 ? theme.accentDeep : theme.clay }}>
                      {changeText(h.dayChange, h.dayChangePct ?? 0)} <span style={{ color: "var(--color-bk-faint)" }}>today</span>
                    </div>
                  ) : h.gain != null ? (
                    <div className="bk-num" style={{ fontSize: 11, marginTop: 2, color: up ? theme.accentDeep : theme.clay }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, minHeight: 200, textAlign: "center" }}>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "var(--color-bk-muted)", maxWidth: 300 }}>
      ...
      {nothing ? (
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: "8px auto 0", fontSize: 14, color: "var(--color-bk-muted)", maxWidth: 400 }}>
        ...
        <>
          ...
          >
            ...
            <div style={{ position: "relative" }}>
              <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--color-bk-faint)" }}>
                Portfolio value
              </div>
              <div
                className="bk-num"
              ...
              <div style={{ marginTop: 14, fontSize: 13, color: "var(--color-bk-muted)" }}>
                {hasAccounts && (
                  <>
          ...
          <section style={CARD}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, margin: "0 0 8px" }}>
              ...
              <span style={{ fontSize: 12, color: "var(--color-bk-faint)", flexShrink: 0 }}>
            ...
            {allocSlices.length === 0 ? (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: 180, fontSize: 13, color: "var(--color-bk-muted)" }}>
              ...
              >
                <DonutChart segments={donutSegments} size={236} stroke={34} formatValue={money} onSelect={activateSlice}>
                  ...
                  <span className="bk-num" style={{ fontSize: centerLabel.length > 10 ? 20 : 26, fontWeight: 500 }}>
                    {money(centerValue)}
                  </span>
                </DonutChart>
                <div
                  ...
                  {allocSlices.map((s, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="bk-num" style={{ fontSize: 14, color: "var(--color-bk-ink)", flexShrink: 0 }}>
                          {money(s.value)}
                        </span>
                        <span style={{ fontSize: 12.5, color: "var(--color-bk-faint)", width: 40, textAlign: "right", flexShrink: 0 }}>
                          {pct(levelPct(s.value))}%
                        </span>
                        <ChevronRight
                          size={15}
                          style={{ flexShrink: 0, color: "var(--color-bk-faint)", opacity: s.drillId ? 1 : 0 }}
                        />
                      </div>
                    );
                  })}
            ...
            {allocNote && (
              <p style={{ margin: "10px 2px 0", fontSize: 12.5, color: "var(--color-bk-muted)" }}>{allocNote}</p>
            )}
            </div>
          </section>
          ...
          {hasAccounts && (
            <section className="bk-grid-2up" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
              {/* accounts */}
              <div style={CARD}>
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: "0 0 6px" }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--color-bk-faint)" }}>{invAccounts.length} total</span>
                </div>
                <div style={{ margin: "0 -4px" }}>
                  {invAccounts.map((a, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {meta && <div style={{ fontSize: 12, color: "var(--color-bk-faint)", marginTop: 2 }}>{meta}</div>}
                        </div>
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          <div className="bk-num" style={{ fontSize: 15, fontWeight: 500, whiteSpace: "nowrap" }}>
                            {money(a.balance)}
                          </div>
                          <div style={{ fontSize: 11, marginTop: 2, color: "var(--color-bk-faint)" }}>{pct(share(a.balance))}%</div>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div style={CARD}>
                <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", margin: "0 0 18px" }}>
                  ...
                  <span style={{ fontSize: 12, color: "var(--color-bk-faint)" }}>Where it&rsquo;s held</span>
                </div>
                <div>
                  {(() => {
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 13, marginBottom: 7, gap: 12 }}>
                          <span
                            style={{
                              ...
                              color: inst.name === "Manual" ? "var(--color-bk-muted)" : undefined,
                            }}
                          >
                            {inst.name}
                          </span>
                          <span className="bk-num" style={{ color: "var(--color-bk-muted)", flexShrink: 0 }}>
                            {money(inst.value)}
                            <span style={{ color: "var(--color-bk-faint)" }}> · {pct(share(inst.value))}%</span>
                          </span>
                        </div>
