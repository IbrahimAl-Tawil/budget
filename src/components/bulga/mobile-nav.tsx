...
// (see .bk-hamburger / .bk-rail in globals.css).
...
function SheetLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-2.5 pb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.09em] text-[var(--color-bk-faint)]">
      {children}
    </div>
  );
}
...
export function MobileNav({
      ...
      <Dialog.Close
        ...
        render={
          <Link
            ...
            className="bk-nav-row flex items-center gap-3 rounded-[14px] px-2.5 py-[7px] text-[15px] font-semibold no-underline outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]/40"
            style={
              active
              ...
              style={
                active
                  ...
                  : {
                      ...
                      boxShadow: "inset 0 0 0 1px var(--color-bk-line-soft)",
                    }
              }
            >
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger
        className="bk-hamburger group relative grid h-10 w-10 cursor-pointer place-items-center rounded-full border border-[var(--color-border)] bg-[var(--color-background)] outline-none transition-colors hover:bg-[var(--color-bk-line-soft)] focus-visible:ring-2 focus-visible:ring-[var(--color-primary)]/40 data-popup-open:bg-[var(--bk-accent)]"
        aria-label="Menu"
      >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup
          ref={popupRef}
          className="bk-sheet fixed inset-x-0 bottom-0 z-[61] mx-auto flex max-h-[86vh] w-full max-w-[520px] flex-col rounded-t-[30px] bg-[var(--color-bk-surface)] pb-[max(16px,env(safe-area-inset-bottom))] outline-none"
        >
          ...
          >
            ...
            <div className="flex items-center gap-2 px-5 pb-3">
              <LogoMark size={30} />
              <span className="bk-num text-[19px] font-medium tracking-[-0.01em] text-[var(--color-bk-ink)]">
                Bulga
              </span>
            </div>
          ...
          <nav aria-label="Primary" className="bk-scroll flex-1 overflow-y-auto px-3 pt-3">
          ...
          <div className="mt-1 px-3 pt-1">
            <div className="rounded-[18px] border border-[var(--color-bk-line-soft)] bg-[var(--color-bk-canvas)] p-2.5">
