      ...
      <GlassCard className="col-span-2">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="font-serif text-[26px] tracking-[-0.02em]">
          ...
          <div className="flex gap-2">
            {surplus > 0 && allocations.length === 0 && (
              <button
                ...
                disabled={isPending}
                className="h-10 px-4 rounded-xl bg-bulga-text text-white text-sm font-semibold hover:opacity-85 disabled:opacity-50"
              >
            ...
            {hasPending && (
              <button
                ...
                onClick={applyAll}
                disabled={isPending}
                className="h-10 px-4 rounded-xl bg-sage text-white text-sm font-semibold hover:opacity-85 disabled:opacity-50"
              >
          ...
          <div className="mt-4 space-y-2">
            {allocations.map((a) => {
              ...
              return (
                <div
                  key={a.id}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-[oklch(100%_0_0/0.1)]"
                >
                  ...
                  {isApplied ? (
                    <span className="text-[13px] font-bold text-sage">{fmt(a.amount)}</span>
                  ) : (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="w-24 px-2 py-1.5 rounded-lg border-[1.5px] border-[oklch(28%_0.012_260/0.14)] bg-[oklch(100%_0_0/0.55)] font-sans text-[13px] text-right text-bulga-text outline-none focus:border-sage"
                      />
                    </div>
                  )}
                </div>
        ...
        return (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span className="text-[10px] font-semibold tracking-[0.06em] uppercase px-2 py-0.5 rounded-full bg-[oklch(100%_0_0/0.18)] text-muted-text">
                {priorityLabel}
              </span>
            </div>
            <div className="text-[22px] font-serif mb-4 tracking-[-0.02em]">{g.emoji} {g.name}</div>
            <div className="flex items-center gap-6 mb-5">
              ...
              <div>
                ...
                <div className="font-serif text-[22px] tracking-[-0.02em]" style={{ color: g.color }}>{fmt(g.saved)}</div>
