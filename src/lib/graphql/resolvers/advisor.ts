...
// Durable per-user monthly cap on advisor messages, to curb abuse/cost. Counted
// from the DB (not the in-memory limiter) so it survives restarts, over the
// current calendar month. The cap comes from the user's plan (see lib/plans
// aiMonthlyMessages) rather than a fixed constant.
...
builder.mutationField("askAdvisor", (t) =>
  t.field({
    ...
    resolve: async (_root, args, ctx) => {
      ...
      const monthlyLimit = entitlements.aiMonthlyMessages;
      ...
      // Durable monthly cap (per-plan): bounds cost/abuse even across restarts,
      // counted from the DB over the current calendar month. A null limit means
      // unlimited (Pro) — skip the cap entirely; the per-minute/hour rate limits
      // above still guard against abuse.
      if (monthlyLimit != null) {
        ...
        if (monthlyCount >= monthlyLimit) {
          rateLimited(
            ...
            `You've reached your plan's limit of ${monthlyLimit} AI messages this month. It resets on ${nextMonthLabel(monthStart)}.`,
          );
        }
      }
