...
// rolling 24h window. The cap now comes from the user's plan (see lib/plans
// aiDailyMessages) rather than a fixed constant.
...
builder.mutationField("askAdvisor", (t) =>
  t.field({
    ...
    resolve: async (_root, args, ctx) => {
      ...
      const dailyLimit = entitlements.aiDailyMessages;
      ...
      // Durable daily cap (per-plan): bounds cost/abuse even across restarts.
      // A null limit means unlimited (Pro) — skip the cap entirely; the
      // per-minute/hour rate limits above still guard against abuse.
      if (dailyLimit != null) {
        const dailyCount = await countAdvisorTurnsSince(userId, new Date(Date.now() - DAY));
        if (dailyCount >= dailyLimit) {
          rateLimited(
            DAY / 1000,
            `You've reached the daily limit of ${dailyLimit} advisor messages. Please try again tomorrow.`,
          );
        }
      }
